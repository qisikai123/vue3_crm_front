...
function onShowCloseClick() {
  addDrawer({
    title: "不显示右上角关闭按钮图标",
    showClose: false,
    contentRenderer: () => <p>不显示右上角关闭按钮图标</p>
  });
}
...
function onBeforeCloseClick() {
  addDrawer({
    ...
    contentRenderer: () => <p>禁止通过键盘ESC关闭</p>
  });
}
...
function onCloseOnClickModalClick() {
  addDrawer({
    ...
    contentRenderer: () => <p>禁止通过点击modal关闭</p>
  });
}
...
function onNestedAddDrawerClick(index: number) {
  addDrawer({
    title: "嵌套抽屉",
    contentRenderer: () => (
      <el-button onClick={() => onNestedAddDrawerClick(index + 1)}>
        打开{index + 1}级嵌套抽屉
      </el-button>
    )
  });
...
<template>
  <el-card shadow="never">
    <template #header>
      ...
      <el-link
        href="https://github.com/qisikai123/vue3_crm_front/tree/master/src/views/components/drawer"
        target="_blank"
      >
    ...
    <el-space wrap>
      ...
      <el-button @click="onNestedAddDrawerClick(0)">嵌套抽屉</el-button>
